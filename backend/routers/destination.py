...
from backend.models import Destination, users, latest_questionnaire
...
def get_destination(destination_id: int, db: Session = Depends(get_db), current_user: user.User = Depends(get_current_user)):
    ...
    response = {
        ...
        # "weather data" : ,
        # "hotel data" : ,
        "cost for bicycle" : cost_for_bicycle(distance_f, latest_questionnaire_of_accessed_user.no_of_people),
        "cost for car" : cost_for_car(distance_f, latest_questionnaire_of_accessed_user.no_of_people),
        "cost for private bus" : cost_for_p_bus(distance_f, latest_questionnaire_of_accessed_user.no_of_people),
        "cost for transit" : cost_for_transit(distance_f, latest_questionnaire_of_accessed_user.no_of_people),
        # "guide details" :
